...
public class UDPClient2 {
    ...
    public void Init() {
        try {
            // 指定端口号，避免与其他应用程序发生冲突
            dataSocket = new DatagramSocket(2112,InetAddress.getByName("127.0.0.1"));
            sendDataByte = new byte[128];
            FileInputStream in=new FileInputStream("D:/CODE/Java/测试/MyTest/src/Test/test1.java");
            //指定要发送的文件
            int i=0;
            long size=new File("D:/CODE/Java/测试/MyTest/src/Test/test1.java").length();
            ...
            while(i!=-1)
            {
                ...
                dataPacket = new DatagramPacket(sendDataByte, sendDataByte.length, InetAddress.getByName("127.0.0.1"), PORT);
                ...
                if(size<128){
                    ...
                    dataPacket = new DatagramPacket(b,b.length, InetAddress.getByName("127.0.0.1"), PORT);
                    dataSocket.send(dataPacket);
                    i=-1;
                    break;
                }
